...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_importance_mask):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration % 1000 == 0:
            gaussians.oneupSHdegree(opt, iteration)
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, opt.iterations, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), render_pkg)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold, 1, scene=scene, pipe=pipe, background=bg, render=render, opt=opt)
...
def training_report(tb_writer, iteration, iter_end, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, render_pkg):
    if tb_writer:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_importance_mask", action="store_true")
    ...
    if run.sweep_id:
        args.model_path += wandb.config.scene
    ...
    t0 = time()
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.use_importance_mask)
    ...
    fps = render_sets(lp.extract(args), args.iteration, pp.extract(args), args.skip_train, args.skip_test)
    ...
    full_dict = evaluate(args.model_paths)
    
    wandb.log({
        "train/training_duration": training_duration,
        "train/fps": fps['train'],
        "test/fps": fps['test'],
        "test/ssim": full_dict[args.model_path]['ours_30000']['SSIM'],
        "test/psnr": full_dict[args.model_path]['ours_30000']['PSNR'],
        "test/lpips": full_dict[args.model_path]['ours_30000']['LPIPS'],
    }, commit=True)